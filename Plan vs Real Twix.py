...
caminho_pasta = r'C:\\Users\\an770843\\Mars Inc\\Brazil_MW_PlanningComex - Documents\\1 - PCP\\2 - Plano de produção\\2025\\Twix\\Compilação Automação BI'

def excel_date(num):
    """Converte um número de série do Excel para uma data do pandas."""
    return pd.to_datetime('1899-12-30') + pd.to_timedelta(num, unit='D')

def reorganizar_dados_twix(caminho_pasta):
    print("Iniciando a reorganização dos dados do Twix...")  # Mensagem de início
    ...
    for arquivo_excel in arquivos:
        ...
        try:
            ...
            df = pd.read_excel(arquivo_excel, sheet_name='3_Heijunka', header=None)
        ...
        periodo = nome_arquivo[17:20]  # Pega os 3 caracteres a partir da posição 17 (índice 16)
        semana = nome_arquivo[nome_arquivo.find('W'):nome_arquivo.find('W') + 2]  # Wx

        # A segunda linha contém as datas
        datas = df.iloc[0, 7:].tolist()  # Pega as datas a partir da 8ª coluna
            ...
            for i in range(len(datas)):
                ...
                if pd.notna(quantidade) and quantidade > 0:  # Verifica se a quantidade é válida
                    ...
                    dados_reorganizados.append({
                        'Máquina': 'Twix',  # Nova coluna adicionada
                        'SKU': sku,
                        ...
                        'Período': periodo,  # Pega os 3 primeiros caracteres para o período
                        'Week': semana,      # Pega o restante para a semana
                        'Ano': ano,  # Nova coluna "Ano" adicionada
                        'Dia Semana': numero_dia_semana,  # Nova coluna "Número do Dia da Semana" adicionada
                        'P/W/Y/D': periodo + semana + ano + str(numero_dia_semana),
        ...
        try:
            df_reorganizado.to_excel(os.path.join(os.path.dirname(caminho_pasta), 'Dados_Organizados_Twix.xlsx'), index=False)
            print(f"Dados reorganizados salvos em: {os.path.join(os.path.dirname(caminho_pasta), 'Dados_Organizados_Twix.xlsx')}")
        except Exception as e:
            print(f"Erro ao salvar o arquivo: {e}")

# Chamando a função reorganizar_dados_twix
reorganizar_dados_twix(caminho_pasta)